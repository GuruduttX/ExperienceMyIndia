          ...
          <div className="hidden md:block relative h-[420px]">
            {destinations.map((item, index) => {
              const stackIndex =
                ...
                destinations.length;
              const isAnimatingThis = stackIndex === 0 && animatingOut;

              const translateX = isAnimatingThis ? 20 : stackIndex * 45;
              const translateY = isAnimatingThis ? -20 : stackIndex * 55;
              const opacity = isAnimatingThis ? 0 : 1 - stackIndex * 0.08;
              const zIndex = isAnimatingThis ? 20 : 10 - stackIndex;

              const transitionClass =
                stackIndex === destinations.length - 1
              ...
              return (
                <div
                  key={index}
                  className={`absolute top-0 left-0 w-[520px] h-[180px] bg-white rounded-2xl shadow-xl border border-gray-100 flex gap-5 p-4 hover:-translate-y-2 ${transitionClass}`}
                  style={{
                ...
                >
                  ...
                  <div className="flex flex-col justify-center">
                    ...
                    <button className="mt-3 flex justify-end text-sm font-medium text-[#FF9933]">
                      Explore →
                    </button>
                  </div>
                </div>
